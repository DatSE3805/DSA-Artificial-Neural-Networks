...
double_tensor MLPClassifier::predict(DataLoader<double, double>* pLoader,
  ...
  {
    // YOUR CODE IS HERE
    double_tensor batch_X = batch.getData();
    double_tensor batch_y = batch.getLabel();
    double_tensor batch_output = forward(batch_X);
    ...
    if (first_batch)
    {
      ...
      results = xt::concatenate(xt::xtuple(results, batch_output), 0);
...
double_tensor MLPClassifier::evaluate(DataLoader<double, double>* pLoader) {
  ...
  for (auto batch : *pLoader)
  {
    ...
    // Update metrics
    meter.accumulate(batch_output, batch_y);
  }
...
void MLPClassifier::backward()
{
  ...
  for (int i = m_layers.size() - 1; i >= 0; i--)
    grad = m_layers.get(i)->backward(grad);
}
