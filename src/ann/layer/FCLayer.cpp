...
xt::xarray<double> FCLayer::forward(xt::xarray<double> X)
{
    ...
    xt::xarray<double> Y = xt::linalg::dot(X, xt::transpose(m_aWeights)); // Y = X x W^T
...
xt::xarray<double> FCLayer::backward(xt::xarray<double> DY)
{
    ...
    m_aGrad_W += xt::linalg::outer(xt::transpose(DY), m_aCached_X);  // ∆W = ∆yT ⊗ x

    if (m_bUse_Bias)
        m_aGrad_b += xt::sum(DY, {0}); // ∆b = ∆Y

    xt::xarray<double> DX = xt::linalg::dot(DY, m_aWeights);        // ∆X = ∆Y × W
