        ...
        */
        TensorDataset(xt::xarray<DType> data, xt::xarray<LType> label) {
            ...
            */
            ...
            data_shape.assign(this->data.shape().begin(), this->data.shape().end());
            label_shape.assign(this->label.shape().begin(), this->label.shape().end());
        }
        ...
        */
        DataLabel<DType, LType> getitem(int index) override {
            ...
            */
            ...
            if (label.size() == 0)
                label_item = xt::xarray<LType>();
            else if (label.dimension() > 1)
                label_item = xt::view(label, index, xt::all());
            else
                label_item = xt::xarray<LType>(label(index));
