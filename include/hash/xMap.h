    ...
    for (auto it = bucket.begin(); it != bucket.end(); it++)
    {
        ...
        {   
            ...
            if (deleteKeyInMap != nullptr)
                deleteKeyInMap(p->key);

            table[index].removeItem(*it, xMap<K, V>::deleteEntry);
...
bool xMap<K,V>::remove(K key, V value, void (*deleteKeyInMap)(K), void (*deleteValueInMap)(V))
{
    ...
    for (auto it = bucket.begin(); it != bucket.end(); it++)
    {
        ...
        if (keyEQ(p->key, key) && valueEQ(p->value, value))
        {
            if (deleteKeyInMap != nullptr)
                deleteKeyInMap(p->key);

            if (deleteValueInMap != nullptr)
                deleteValueInMap(p->value);
...
{
    //YOUR CODE IS HERE 
    DLinkedList<int> ListOfClashes;

    for (int i = 0; i < capacity; i++)
        ListOfClashes.add(table[i].size());
