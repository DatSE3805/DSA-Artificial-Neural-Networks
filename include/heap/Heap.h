...
void Heap<T>::remove(T item, void (*removeItemData)(T))
{
    ...
    if (removeItemData != nullptr)
        removeItemData(elements[position]);
...
{
    //YOUR CODE IS HERE
    int Index_Parent = (position - 1) / 2;
    
    while (position > 0 && aLTb(elements[position], elements[Index_Parent]))
    {
        std::swap(elements[position], elements[Index_Parent]);
        position = Index_Parent;
        Index_Parent = (position - 1) / 2;
    }
}
...
void Heap<T>::reheapDown(int position)
{
    ...
    while (true)
    {
        int L_index = 2 * current + 1;
        int R_index = 2 * current + 2;
        int max = current;

        if (L_index < count && aLTb(elements[L_index], elements[max]))
            max = L_index;
        
        if (R_index < count && aLTb(elements[R_index], elements[max]))
            max = R_index;

        if (max == current)
            break;

        std::swap(elements[current], elements[max]);
        current = max;
    }
}
